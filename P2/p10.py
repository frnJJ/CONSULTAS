...
def comprimir_notas (nombres, nota1, nota2): 
    ...
    est_notas = tuple(zip(aux,nota1,nota2))
    return est_notas
...
def max_promedio(datos_alu):
    ...
    aux = calcular_promedio(datos_alu)
    aux = list(aux)
...
def min_nota(datos_alu):
    ...
    minnotas = map(lambda x:((x[0],x[1]) if x[1] <= x[2] else (x[0],x[2])),datos_alu)
    return sorted(minnotas, key=(lambda x: x[1]))[0]
